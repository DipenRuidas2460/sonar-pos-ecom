...
export default function useEcomerce() {
    ...
    return {
        ...
        addItem: (newItem, items, group) => {
            ...
            if (group === 'wishlist') {
                // console.log("newItems wishlist===>", newItems);
                // setCookie('wishlist', newItems, { path: '/' });
                // dispatch(setWishlistTtems(newItems));

                axios
                    .post(
                        `${ProductGetUrl}/${baseProductUrl}/wishlistinsert`,
                        newItem,
                        {
                            headers: {
                    ...
                    .then((res) => {
                        if (res.data.ResponseCode === 'SUCCESS') {
                            ...
                            else
                            {
                                notification['success']({
                                    message: 'Success!',
                                    description: 'Item removed to your wishlist!',
                                });

                                // let currentItems = newItem;
                            ...
                            // toastError('Email' + ALREADY_EXIST);
                        } else {
                            notification['error']({
                                message: 'Opps!',
